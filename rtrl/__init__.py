...
def run_fs(path: str, run_cls: type = Training):
    ...
    # initialize an empty DataFrame if this is the first run
    if not exists(path + '/stats'):
        dump(pd.DataFrame(), path + '/stats')

    # for each batch of episodes, append the new stats via concat
    for stats in iterate_episodes(run_cls, path + '/state'):
        old = load(path + '/stats')                      # load previous stats
        new_df = pd.DataFrame(stats)              # turn list of pd.Series → DataFrame
        combined = pd.concat([old, new_df], ignore_index=True)
        dump(combined, path + '/stats')                  # overwrite with updated DataFrame
